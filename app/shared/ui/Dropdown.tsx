...
export function Dropdown({ options, selectedId, onSelect, placeholder = "Seleccionar...", className = "" }: DropdownProps) {
  ...
  return (
    <div className={`relative ${className}`} ref={dropdownRef}>
      ...
      {isOpen && (
        <div className="absolute top-[calc(100%+8px)] left-0 right-0 z-50 bg-brand-secondary border border-brand-secondary rounded-xl overflow-hidden shadow-xl animate-in fade-in slide-in-from-top-1">
          {options.map((option) => (
            <button
              key={option.id}
              onClick={() => {
