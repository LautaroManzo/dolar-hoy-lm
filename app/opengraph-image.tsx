...
export const alt = 'DolarInfoHoy - Cotización del Dólar Blue en Argentina'
export const size = { width: 1200, height: 630 }
export const contentType = 'image/png'
export const revalidate = 60

interface DolarRaw { casa: string; compra: number; venta: number; variacion: number }

async function getBlue(): Promise<DolarRaw | null> {
  try {
    const res = await fetch('https://dolarapi.com/v1/ambito/dolares', {
      next: { revalidate: 60 },
    })
    if (!res.ok) return null
    const data: DolarRaw[] = await res.json()
    return data.find(d => d.casa === 'blue') ?? null
  } catch {
    return null
  }
}
...
export default async function Image() {
  ...
  const variacion = blue?.variacion ?? 0
  const variacionText = variacion === 0 ? 'Sin variación' : `${variacion > 0 ? '▲' : '▼'} ${Math.abs(variacion).toFixed(1)}%`
  const variacionColor = variacion > 0 ? '#4ade80' : variacion < 0 ? '#f87171' : 'rgba(255,255,255,0.4)'
      ...
      <div
        style={{
          ...
          backgroundImage: `
            ...
            radial-gradient(circle at 15% 90%, rgba(45,90,123,0.35) 0%, transparent 45%),
      ...
      >
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', width: '100%' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '16px' }}>
            <svg width="52" height="52" viewBox="0 0 25 25" fill="none" xmlns="http://www.w3.org/2000/svg">
              ...
              <path d="M12.58 3.96997H6C4.93913 3.96997 3.92178 4.39146 3.17163 5.1416C2.42149 5.89175 2 6.9091 2 7.96997V17.97C2 19.0308 2.42149 20.0482 3.17163 20.7983C3.92178 21.5485 4.93913 21.97 6 21.97H18C19.0609 21.97 20.0783 21.5485 20.8284 20.7983C21.5786 20.0482 22 19.0308 22 17.97V11.8999" stroke="#ffffff" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/>
              <path d="M16.3398 8.57992L21.9998 2.91992" stroke="#ffffff" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/>
              <path d="M17.4805 2.91992H22.0005V7.44992" stroke="#ffffff" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/>
            </svg>
            <span style={{ color: 'white', fontSize: '34px', fontWeight: 700, letterSpacing: '-0.5px' }}>
              DolarInfoHoy
            </span>
          </div>
          <div style={{
            ...
            <div style={{ width: '10px', height: '10px', borderRadius: '50%', background: '#4ade80', boxShadow: '0 0 8px #4ade80', display: 'flex' }} />
            <span style={{ color: 'white', fontSize: '20px', fontWeight: 600 }}>EN VIVO</span>
          </div>
        </div>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '16px' }}>
          ...
          <span style={{ color: 'white', fontSize: '120px', fontWeight: 800, lineHeight: 1, letterSpacing: '-4px' }}>
            ${fmt(blue?.venta)}
          </span>
          <span style={{ color: variacionColor, fontSize: '30px', fontWeight: 700 }}>
            {variacionText}
          </span>
        </div>
